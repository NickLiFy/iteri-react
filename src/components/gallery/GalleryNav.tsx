...
export const GalleryNav = ({ activeCat }: GalleryNavProps) => {
    ...
    return (
        <nav className="fixed top-20 left-0 w-full z-40 bg-brand-bg/80 backdrop-blur-xl border-b border-brand-white/5 py-4">
            <div className="container mx-auto px-6 flex flex-wrap gap-4 items-center justify-center">
                {Object.entries(servicesConfig).map(([key, config]) => (
                    <button
                        ...
                        className={`cursor-pointer px-5 py-2.5 rounded-full text-[10px] font-black uppercase tracking-[0.15em] transition-all duration-300 ${
                            activeCat === key
